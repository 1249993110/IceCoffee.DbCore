...
namespace IceCoffee.DbCore.CodeGenerator.UserControls
{
    public partial class UC_PostgresSQL : UserControl, IView
    {
        ...
        private void listView_entities_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (this.listView_entities.SelectedItems.Count > 0)
                {
                    ...
                    this.Preview(entityName, fieldInfos);
                }
            }
            catch (Exception ex)
            {
        ...
        private void Preview(string entityName, IEnumerable<FieldInfo> fieldInfos)
        {
            bool isView = DBHelper.ExecuteScalar<string>(_dbConnectionInfo, $"SELECT relkind FROM pg_class WHERE relname='{entityName}'") == "v";
